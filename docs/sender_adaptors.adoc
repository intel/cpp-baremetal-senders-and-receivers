...
auto then_sndr = async::then(sndr,
    ...
    [] (int j) { return j + 1; });
// when run, then_sndr will send "42" and 43
----
...
In the variadic case, `then` distributes the values sent from upstream to the
functions https://intel.github.io/cpp-std-extensions/#_call_by_need_hpp[according to need]:
...
CAUTION: Beware of implicit argument conversions. See the documentation for
