...
template <typename Ops, typename Rcvr> struct receiver {
    ...
    auto set_error(Args &&...args) const && -> void {
        ops->template complete<set_error_t>(std::forward<Args>(args)...);
    }
    ...
    template <typename... Args> auto repeat(Args &&...args) -> void {
        if constexpr (not std::same_as<
            ...
            debug_signal<"eval_predicate", debug::erased_context_for<op_state>>(
                get_env(rcvr));
            if (pred(args...)) {
                complete<set_value_t>(std::forward<Args>(args)...);
                return;
            }
        }
        setup();
...
struct sync_op_state {
    ...
    // NOLINTNEXTLINE(bugprone-forwarding-reference-overload)
    constexpr sync_op_state(S &&s, R &&r, P &&p, auto &&)
        : sndr{std::forward<S>(s)}, rcvr{std::forward<R>(r)},
    ...
    constexpr auto start() & -> void {
        ...
        // cause a call to either repeat or complete. Therefore we know that the
        ...
        // receiver (in the case of error or stopped) or from repeat (in the
        // case of success).
    }
    ...
    template <typename... Args> auto repeat(Args &&...args) -> void {
        ...
        // By not calling set_value here, we go around the loop in start.
    }
...
constexpr auto select_op_state() {
    if constexpr (synchronous<connect_result_t<
        ...
        return std::type_identity<op_state<Name, Sndr, Rcvr, Pred, LoopFn>>{};
    }
}
...
struct sender {
    ...
    template <typename R>
    using op_state_for =
        detail::op_state_t<Name, Sndr, std::remove_cvref_t<R>, Pred, LoopFn>;
...
template <stdx::ct_string Name, typename... Ts>
struct debug::context_for<_repeat::sync_op_state<Name, Ts...>>
    : debug::context_for<_repeat::op_state<
          Name, Ts..., std::remove_cvref_t<decltype(_repeat::no_loop_fn)>>> {};
} // namespace async
